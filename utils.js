...
const twilioClient = require('twilio')(process.env.TWILIO_ACCOUNT_SID, process.env.TWILIO_AUTH_TOKKEN)
...
const sendEmail = async ({ email, redirectTo }) => {
	try {
		...
		const emailRes = await sgMail.send({
			...
			subject: 'Verify user',
			text: 'please verify your self so we can transfer money safely',
